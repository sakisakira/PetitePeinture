...
void CanvasController::setupPainter(void) {
  ...
  NSLog(@"CanvasController::setupPainter() started");
...
//  tools = sp->getTools();
  ...
  NSInteger num_of_layers = [[NSUserDefaults standardUserDefaults]
			      integerForKey:[kNumOfLayersKey copy]];
  if (num_of_layers > 0) {
    loadDefaults();
  } else {
    clearImage(false);
...
void CanvasController::saveImage(void) {
  ...
  [archiver encodeInt:num_of_layers
	    forKey:[kNumOfLayersKey copy]];
  [archiver encodeInt:sp->get_width()
        forKey:[kImageWidthKey copy]];
  [archiver encodeInt:sp->get_height()
	    forKey:[kImageHeightKey copy]];
...
void CanvasController::loadImage(NSString *fpath) {
  ...
  [data release];
  NSInteger num_of_layers = [arc decodeIntForKey:[kNumOfLayersKey copy]];
  if (num_of_layers <= 0) {
    [arc finishDecoding];
    [arc release];
    return;
  ...
  width = [arc decodeIntForKey:[kImageWidthKey copy]];
  height = [arc decodeIntForKey:[kImageHeightKey copy]];
...
void CanvasController::loadDefaults(void) {
  ...
  NSInteger num_of_layers = [defs integerForKey:[kNumOfLayersKey copy]];
  if (num_of_layers <= 0) return;

  NSInteger width, height;
  width = [defs integerForKey:[kImageWidthKey copy]];
  height = [defs integerForKey:[kImageHeightKey copy]];
  sp->setSize(width, height);
  ...
  [[defs dataForKey:[kPencilKey copy]]
    getBytes:&pencil length:sizeof(pencil)];
  [[defs dataForKey:[kBrushKey copy]]
    getBytes:&brush length:sizeof(brush)];
  [[defs dataForKey:[kEraserKey copy]]
    getBytes:&eraser length:sizeof(eraser)];
  [[defs dataForKey:[kCloudKey copy]]
    getBytes:&cloud length:sizeof(cloud)];

  set_pen();
  display_pen();

  sp->setLayer([defs integerForKey:[kCurrentLayerIndexKey copy]]);
  sp->updateRect(PtRect(0, 0, width, height));
}
...
//    [ptview setNeedsDisplay];
  ...
  [ptview setNeedsDisplayInRect:CGRectMake(rect.x / s_scale, 
																					 ...
																					 rect.w / s_scale, 
																					 rect.h / s_scale)];
//	NSLog(@"update rect (%d, %d) %d x %d", rect.x, rect.y, rect.w, rect.h);
}
...
void CanvasController::mousePressEvent(const PtPair &orig_e) {
  if (this->_painterWrapper->_drawing) return;
...
//      updateRect(rect);
      ...
      PenStroke *stroke = [[PenStroke alloc] init];
      stroke->pen = ppen;
      stroke->rect = rect;
      stroke->x0 = x0;
      stroke->y0 = y0;
      ...
      @synchronized(this->_painterWrapper.queuedStrokes) {
        ...
        if (!this->_painterWrapper->_drawing)
...
//  layers.editing = l = layers.layerNum(opt_panel_rect.h - y);
  ...
  //  NSLog(@"CanvasController::layerpanel_event %d %d", y, l);
...
void CanvasController::layerpanel_release_event(const PtPair &e) {
  ...
  y = e.y - opt_panel_rect.y;
  l = layers.layerNum(y);
  //  NSLog(@"CanvasController::layerpanel_release_event: layernum: %d", l);
...
void CanvasController::pick_color(const PtPair &e) {
  PtPair p(convert_geometry(e));
...
//  NSLog(@"release (%d - %d) (%d - %d)", x0, x1, y0, y1);
...
//  NSLog(@"clipboardrect %d %d %d %d, pt : %d %d",
